...
import com.vietinterview.getbee.adapter.MyResumeSubmitAdapter;
 ...
 */
public class MyResumeApplyedFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener, OnLoadMoreListener {
    @BindView(R.id.swipe_container)
    SwipeRefreshLayout mSwipeRefreshLayout;
    private ArrayList<CvList> cvLists = new ArrayList<>();
    private ArrayList<CvList> cvListsServer = new ArrayList<>();
    private MyResumeSubmitAdapter myCVSubmitAdapter;
    ...
    public static MyResumeApplyedFragment newInstance(int status, int carrerId, int cityId) {
        MyResumeApplyedFragment fm = new MyResumeApplyedFragment();
...
////                    jobsList.remove(jobsList.size() - 1);
                    ...
                    if (page == 0) {
                        myCVSubmitAdapter = new MyResumeSubmitAdapter(recyclerView, getActivity(), data.getTotal(), cvLists, MyResumeApplyedFragment.this);
                        recyclerView.setAdapter(myCVSubmitAdapter);
                    }
                    myCVSubmitAdapter.setOnLoadMoreListener(MyResumeApplyedFragment.this);

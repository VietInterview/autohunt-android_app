...
import com.vietinterview.getbee.adapter.MyResumeSavedAdapter;
 ...
 */
public class MyResumeSavedFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener, OnLoadMoreListener {
    ...
    private MyResumeSavedAdapter myCVSavedAdapter;
    ...
    public static MyResumeSavedFragment newInstance(int carrerId, int cityId) {
        MyResumeSavedFragment fm = new MyResumeSavedFragment();
        ...
        searchCVSaveRequest.callRequest(getActivity(), new ApiObjectCallBack<CVResponse, ErrorResponse>() {
            ...
            public void onSuccess(int status, CVResponse data, List<CVResponse> dataArrayList, String message) {
                ...
                if (isAdded()) {
                    ...
                    if (page == 0) {
                        myCVSavedAdapter = new MyResumeSavedAdapter(recyclerView, getActivity(), data.getTotal(), cvLists, MyResumeSavedFragment.this);
                        recyclerView.setAdapter(myCVSavedAdapter);
                    }
                    myCVSavedAdapter.setOnLoadMoreListener(MyResumeSavedFragment.this);
